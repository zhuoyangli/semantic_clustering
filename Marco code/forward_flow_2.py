...
def compute_exponentially_weighted_forward_flow(word_sequence, glove_embeddings, decay_factor=0.5):
    ...
    start_index = 0  # This will track the start of the current segment

    for i in range(1, n):  # Start from the second word
        weighted_sum = 0
        weight_total = 0
        
        # Apply exponential decay for each previous word in the current segment
        for j in range(start_index, i):  # For each word before the current word within the current segment
        ...
        if forward_flow_score > 0.7:
            ...
            start_index = i  # Reset the start index to the current word for the next segment
...
if __name__ == "__main__":
    ...
    forward_flow_scores, boundaries = compute_exponentially_weighted_forward_flow(word_chain, glove_vectors, decay_factor)

    if forward_flow_scores is not None:
        for i, score in enumerate(forward_flow_scores, start=1):
            print(f"Forward Flow Score for word {word_chain[i]}: {score:.4f}")
